...
public class D02CheckedExceptionTest {
   //File 클래스: 운영체제의 파일시스템 클래스
    ...
    public static void main(String[] args) {
        ...
        //File file = new File("F:\\테스트.md"); 없는 드라이브 지정->예외
        File file = new File("테스트2.md");
        //디렉토리 지정하지않으면 프로젝트와 같은 폴더에 생성
       ...
       try {
        //if(file.exists())
            ...
            file.createNewFile();
            System.out.println("파일이 길이: "+file.length());
       } catch (Exception e) {  //IOException 처리 : 대표적인 체크 익셉션
        //예외 객체는 변수 e로 참조
        System.out.println("파일 생성에 오류가 발생했습니다.");
        System.out.println(e.getMessage());// 오류원인
       } 
