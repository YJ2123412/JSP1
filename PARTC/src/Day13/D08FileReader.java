...
public class D08FileReader {
    ...
    public static void readByChar(){
       ...
       try( FileReader fr = new FileReader(filePath)){
         ...
         while ((charValue = fr.read()) != -1) {    //read는 문자단위 입력. 문자를 1개씩 읽음(입력)
             ...
             //System.out.print((char)charValue);//문자 단위이기에 표준 출력 사용가능 //파일에서 줄바꿈 =2byte \r\n
             count++;
        }   
    } catch (Exception e) { }
        // FileReader 닫기
    ...
    public static void readByCharArray(){
       ...
       try( FileReader fr = new FileReader(filePath)){
         ...
         while ((b = fr.read(charValues,0,256)) != -1) { //read(배열이름, 시작인덱스, 최대 문자길이)
            //읽은 문자의 개수는 리턴, b 에 저장
    ...
    public static void readByCharBuffer(){
        String filePath = "C:\\Windows\\PFRO.log";
        //한 줄이 256문자보다 많이 적으면 위의 char배열보다 시간이 더 걸림 
        int count=0; //읽은 라인수. 문자기반 버퍼스트림은 라인(줄) 단위로 입출력 기능이 있음
        //int b;
        try (
    ...
    public static void readByScanner2(){
        String filePath = "testSample.txt";
        int count=0;
        ...
        System.out.println("읽은 객체 : "+count);
        for(JavaWord word : list) System.out.println(word);
    }   
}
